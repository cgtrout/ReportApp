...
namespace ProxCard2
{
     ...
     static public class Extractor
     {
          public enum CARD_TYPE { Fob, OldCompCard,  MainCompany34, Invalid }

          #region Methods

          /// <summary>
          /// Scan card data, edits a person
          /// - used by AddPersonViewModel and SearchPersonViewModel
          /// </summary>
          /// <param name="cardData"></param>
          /// <param name="p"></param>
          /// <param name="readMode"></param>
          public static void ScanCard(byte[] cardData, PersonViewModel p, bool readMode)
          {
               var rawHex = ProxCard2.SCARD.ToHex(cardData, "");
               var hexLength = rawHex.Length;

               ProxCard2.Extractor.ExtractFacCodeNumber(rawHex, hexLength, out Extractor.CARD_TYPE cardType, out long facilityCode, out long number);

               TraceEx.PrintLog($"Hex={rawHex} F={facilityCode} N={number} cardType={cardType} scanned from reader");

               if (readMode) {
                    p.AutoChangeCredential = false;
                    if (cardType == Extractor.CARD_TYPE.Fob) {
                         p.FobCredential = $"Facility Code {facilityCode}";
                    } else if (cardType == Extractor.CARD_TYPE.OldCompCard) {
                         p.FobCredential = "OldComp_Power Card";
                    } else if (cardType == Extractor.CARD_TYPE.NewComp34) {
                         p.FobCredential = "34-bit HID Card 988f";
                    }

                    if (cardType != Extractor.CARD_TYPE.Invalid) {
                         p.FobNumber = number;
                         readMode = false;

                         TraceEx.PrintLog("AddPerson: Scanned data entered into fields");
                    }
                    SystemSounds.Beep.Play();

                    p.AutoChangeCredential = true;
               }
          }

          /// <summary>
          /// Extracts facility code and number from raw data (from card reader)
          /// </summary>
          public static void ExtractFacCodeNumber(string rawHex, int hexLength, out CARD_TYPE cardType, out long facilityCode, out long number)
          {
               facilityCode = 0;
               number = 0;
               ...
               if (hexLength == 24) {
                    cardType = CARD_TYPE.Fob;
                    binaryString = hex2bin(rawHex.Substring(14, 7));
                    facilityCode = Convert.ToInt32(binaryString.Substring(1, 8), 2);
                    number = Convert.ToInt32(binaryString.Substring(9, 16), 2);
               } else if (hexLength == 26 && rawHex.StartsWith("3B890100030605")) {
                    cardType = CARD_TYPE.OldCompCard;
                    binaryString = hex2bin(rawHex.Substring(14, 10));
                    facilityCode = Convert.ToInt64(binaryString.Substring(1, 13), 2);
                    number = Convert.ToInt64(binaryString.Substring(14, 21), 2);
               } else if (hexLength == 26 && rawHex.StartsWith("3B890100030606")) {
                    cardType = CARD_TYPE.MainCompany34;
                    binaryString = hex2bin(rawHex.Substring(14, 10));
                    facilityCode = Convert.ToInt64(binaryString.Substring(23, 10), 2);
                    number = Convert.ToInt64(binaryString.Substring(1, 16), 2);
               } else {
                    MessageBox.Show("Invalid card type", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    cardType = CARD_TYPE.Invalid;
               }
          }

          public static string hex2bin(string value)
          {
               return Convert.ToString(Convert.ToInt64(value, 16), 2).PadLeft(value.Length * 4, '0');
          }
     ...
     public class ReaderList : IDisposable, IEnumerable<string>
     {
          ...
          private volatile bool bStopThread = true;
          ...
          public void Dispose()
          {
               TraceEx.PrintLog("Disposing ProxCard::ReaderList");
               StopThread();
          }
          ...
          private void Run()
          {
               ...
               try {
                    ...
                    Trace.TraceError($"{ex.GetType()} exception: {ex.Message}");
               }
               finally {
                    if (hContext != IntPtr.Zero)
                         SCARD.ReleaseContext(hContext);
                    thread = null;
                    TraceEx.PrintLog("Prox: Exiting thread");
               }
          }
